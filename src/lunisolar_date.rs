...
impl LunisolarDate {
    ...
    pub(crate) fn from_solar_date_inner(solar_date: SolarDate, naive_date: NaiveDate) -> Result<LunisolarDate, LunisolarError> {
        if naive_date < *MIN_LUNAR_DATE_IN_SOLAR_CALENDAR || naive_date > *MAX_LUNAR_DATE_IN_SOLAR_CALENDAR {
            ...
            let (leap_month, leap_days, new_year_diff) = if year == MAX_YEAR_IN_SOLAR_CALENDAR {
                ...
                let new_year_diff = NEW_YEAR_DIFFERENCE[(year - MIN_YEAR_IN_SOLAR_CALENDAR) as usize] as u16;

                let leap_lunar_month = lunisolar_year.get_leap_lunar_month();

                match leap_lunar_month {
                    Some(leap_lunar_month) => (leap_lunar_month.to_u8(), lunisolar_year.get_total_days_in_leap_month_inner(leap_lunar_month), new_year_diff),
                    None => (0, 0, new_year_diff)
                }
            };

            if day_diff < new_year_diff { // 若天數差距比「西曆新年與對應農曆年新年」之天數差距小，表示此西曆日期尚未進入下一個農曆年(尚未到達正月，還在農曆年尾)。
                let lunisolar_year = unsafe { LunisolarYear::from_solar_year_unsafe(year - 1) };

                let leap_lunar_month = lunisolar_year.get_leap_lunar_month();

                let (leap_month, leap_days) = match leap_lunar_month {
                    Some(leap_lunar_month) => (leap_lunar_month.to_u8(), lunisolar_year.get_total_days_in_leap_month_inner(leap_lunar_month)),
                    None => (0, 0)
                };
                ...
                loop {
                    ...
                    let month_days = lunisolar_year.get_total_days_in_a_month(unsafe { LunarMonth::from_u8_unsafe(month, false) }).unwrap() as u16;
                ...
                if day_diff == 0 {
                    ...
                    Ok(LunisolarDate {
                        ...
                        lunisolar_year,
                        lunar_month,
                        lunar_day: unsafe { LunarDay::from_u8_unsafe(lunar_month.get_total_days(lunisolar_year).unwrap() - day_diff as u8 + 1) },
                    })
                }
            } else { // 若天數差距沒比「西曆新年與對應農曆年新年」之天數差距小，表示此西曆日期已經進入下一個農曆年(已到達正月，從在農曆年頭開始)。
    ...
    pub unsafe fn from_lunisolar_year_lunar_month_day_unsafe(lunisolar_year: LunisolarYear, lunar_month: LunarMonth, lunar_day: LunarDay) -> LunisolarDate {
        ...
        let solar_year = if n + NEW_YEAR_DIFFERENCE[(year - MIN_YEAR_IN_SOLAR_CALENDAR) as usize] as u16 >= solar_year.get_total_days() {
    ...
    pub fn from_lunisolar_year_lunar_month_day(lunisolar_year: LunisolarYear, lunar_month: LunarMonth, lunar_day: LunarDay) -> Result<LunisolarDate, LunisolarError> {
        ...
        if lunar_day.to_u8() > days {
            ...
            Ok(unsafe {
                Self::from_lunisolar_year_lunar_month_day_unsafe(lunisolar_year, lunar_month, lunar_day)
            })
        }
    }
    ...
    pub(crate) fn the_n_day_in_this_year_inner(lunisolar_year: LunisolarYear, lunar_month: LunarMonth, lunar_day: LunarDay) -> u16 {
        ...
        if lunar_month.is_leap_month() {
            for i in 1..=month {
                ...
                n += lunar_month.get_total_days(lunisolar_year).unwrap() as u16;
            }
        } else {
            for i in 1..month {
                let lunar_month = unsafe { LunarMonth::from_u8_unsafe(i, false) };
